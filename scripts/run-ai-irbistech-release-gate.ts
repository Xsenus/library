...
async function main(): Promise<void> {
  ...
  const summary = await runAiIrbistechReleaseGate({
    ...
    aiIntegrationPythonExecutable:
      ...
      process.env.AI_IRBISTECH_RELEASE_GATE_AI_INTEGRATION_PYTHON_BIN,
    aiSiteAnalyzerPythonExecutable:
      readOption(args, '--ai-site-analyzer-python') ??
      process.env.AI_IRBISTECH_RELEASE_GATE_AI_SITE_ANALYZER_PYTHON_BIN,
    npmExecutable:
      readOption(args, '--npm') ??
      process.env.AI_IRBISTECH_RELEASE_GATE_NPM_BIN,
    workspaceRoot:
      readOption(args, '--workspace-root') ??
      process.env.AI_IRBISTECH_RELEASE_GATE_WORKSPACE_ROOT,
    libraryRoot:
      readOption(args, '--library-root') ??
      process.env.AI_IRBISTECH_RELEASE_GATE_LIBRARY_ROOT,
    aiIntegrationRoot:
      readOption(args, '--ai-integration-root') ??
      process.env.AI_IRBISTECH_RELEASE_GATE_AI_INTEGRATION_ROOT,
    aiSiteAnalyzerRoot:
      readOption(args, '--ai-site-analyzer-root') ??
      process.env.AI_IRBISTECH_RELEASE_GATE_AI_SITE_ANALYZER_ROOT,
    libraryBaseUrl:
