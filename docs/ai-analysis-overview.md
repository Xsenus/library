...
#### Что нужно реализовать на стороне API
Для корректной синхронизации и восстановления после сбоев API должен возвращать в `/api/ai-analysis/companies` по каждой активной компании:
1. `analysis_status` — минимум состояния `queued` / `running` / terminal (`completed`/`failed`/`stopped`).
2. `analysis_started_at` и/или `queued_at` — ISO timestamp для fallback-таймера.
3. `analysis_duration_ms` — фактически накопленная длительность на стороне backend (монотонно неубывающая в рамках текущего запуска).
4. `analysis_finished_at` для terminal-состояний, чтобы UI прекращал локальный tick и фиксировал финальное значение.
5. (Рекомендуется) `analysis_progress` в диапазоне `0..1` для прогресс-бара.

Рекомендации для backend-логики:
- `analysis_duration_ms` лучше считать на сервере от стабильного источника времени (монотонные часы/единый host), обновлять при каждом heartbeat шага.
- Для нового запуска компании обнулять `analysis_duration_ms` и выставлять новый `analysis_started_at`.
- Не возвращать уменьшенное `analysis_duration_ms` в рамках одного запуска (иначе в UI возможны визуальные скачки назад).
