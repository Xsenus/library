...
function isDataLikeUrl(url: string | null | undefined) {
  if (!url) return false;
  return url.startsWith('data:') || url.startsWith('blob:');
}

function resolveUrl(url: string | null | undefined): URL | null {
  if (!url || isDataLikeUrl(url)) return null;
  try {
...
function cloneNodeDeep(node: Element, filter?: (node: HTMLElement) => boolean): Element | null {
  ...
  if (clone instanceof HTMLImageElement) {
    const source = node instanceof HTMLImageElement ? node.currentSrc || node.src : clone.src;
...
async function fetchResource(url: string): Promise<Response> {
  const absolute = resolveUrl(url);
  const isCrossOrigin = absolute ? absolute.origin !== window.location.origin : false;
