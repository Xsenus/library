...
function scrubStyleElement(style: HTMLStyleElement) {
  const cssText = style.textContent;
  if (!cssText || !cssText.includes('url(')) return;

  const sanitized = cssText.replace(/url\((['"]?)([^'"\)]+)\1\)/gi, (match, quote, raw) => {
    ...
    if (absolute && isFetchableResource(absolute)) {
      return match;
    }
...
async function inlineImageElement(
  ...
  if (!isFetchableResource(absoluteUrl)) {
    target.removeAttribute('srcset');
    target.removeAttribute('sizes');
...
async function inlineCssUrls(
  ...
  while ((match = regex.exec(value)) !== null) {
    ...
    if (!absolute) {
      ...
      result += `url("${absolute}")`;
      changed = true;
    } else if (!isFetchableResource(absolute)) {
...
function isFetchableResource(url: string): boolean {
  if (!url) return false;
...
async function fetchAsDataUrl(url: string): Promise<string> {
  ...
  if (!isFetchableResource(absolute)) {
  ...
  if (isSameOrigin(absolute)) {
    const response = await fetch(absolute, { cache: 'no-store', credentials: 'include' });
    if (!response.ok) {
  ...
  const proxied = `/api/snapshot-proxy?url=${encodeURIComponent(target.toString())}`;
  ...
  while (current) {
    if (current instanceof HTMLImageElement) {
      ...
      if (!current.src.startsWith('data:')) {
        current.setAttribute('src', TRANSPARENT_PIXEL);
